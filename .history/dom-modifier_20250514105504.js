...
function modifyDOM() {
  ...
  function walkAndReplaceText(node) {
    if (node.nodeType === Node.TEXT_NODE) {
      if (node.textContent.includes('zue')) {
        node.textContent = node.textContent.replace(/zue/g, 'zuk');
      }
    } else if (node.nodeType === Node.ELEMENT_NODE && node.shadowRoot == null) {
  ...
  // 更新各个容器的内容
  const mobileLogoContainer = document.querySelector('.MobileNav_logoWrapper__zEKug');
  const desktopLogoContainer = document.querySelector('.DesktopNav_logoContainer__yLiJx');
  ...
  const homePedestalMaskedImage = document.querySelector('.Home_pedestalMaskedImage__Hjbwl');
  const hoverCardsNoiseWrapper = document.querySelector('.HoverCards_noiseWrapper__WEJNP');
  const homeFeatureVideoMask = document.querySelector('.Home_featureVideoMask__3bea8');
  const desktopNavRightTabs = document.querySelector('.DesktopNav_rightTabs__ZNHO2');
  ...
  if (homePedestalMaskedImage) {
    ...
    homePedestalMaskedImage.style.maskImage = 'url("https://zue.ai/images/pedestal-mask.svg")';
  ...
  if (desktopNavRightTabs && !desktopNavRightTabs.querySelector('.twitter-button-added')) {
    desktopNavRightTabs.insertAdjacentHTML('beforeend', `
...
function waitForAllResources() {
  return new Promise((resolve) => {
    ...
    const checkScripts = () => {
      ...
      const allLoaded = nextScripts.every(script => 
