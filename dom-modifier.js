...
function modifyDOM() {
  ...
  const homeMobileMediaWrapper = document.querySelector(
  ...
  const homeMobileBg = document.querySelector('.Home_mobileBg__40Qyp');

  if (homeMobileBg) {
    homeMobileBg.innerHTML = `
      <img
        alt=""
        ...
        decoding="async"
        data-nimg="1"
        class="Image_placeholder__uf5AB Image_themeLight__GCSlz"
        style="color: transparent"
        src="https://zue.ai/_next/image?url=%2Fimages%2Fnew_hero_mobile.webp&w=3840&q=90"
      />
    `;
  }

  if (homeMobileMediaWrapper) {
    const laptopMedia = homeMobileMediaWrapper.querySelector(
      '.Home_laptopMedia__IPPL1'
    );
    const pedestalMaskedImage = homeMobileMediaWrapper.querySelector(
      '.Home_pedestalMaskedImage__Hjbwl'
    );
    const featureVideoMask = homeMobileMediaWrapper.querySelector(
      '.Home_featureVideoMask__3bea8'
    );
    if (laptopMedia) {
      laptopMedia.innerHTML = `
    ...
    if (pedestalMaskedImage) {
      const last = pedestalMaskedImage.lastElementChild;
      pedestalMaskedImage.style.maskImage =
